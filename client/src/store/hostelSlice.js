...
// Get Public Colleges for Hostel creation dropdown (URL FIXED)
export const fetchPublicColleges = createAsyncThunk('hostel/fetchCollegesDropdown', async (districtId, { rejectWithValue }) => {
  try {
    const res = await api.get(`/locations/common/colleges/${districtId}`); return res.data.data;
  } catch (err) { return rejectWithValue(err.response?.data?.message); }
});

export const fetchHostels = createAsyncThunk('hostel/fetchHostels', async (_, { rejectWithValue }) => {
  try {
    const res = await api.get('/locations/admin/hostels'); return res.data.data;
  } catch (err) { return rejectWithValue(err.response?.data?.message); }
});

export const addNewHostel = createAsyncThunk('hostel/addNewHostel', async (data, { rejectWithValue }) => {
  try {
    const res = await api.post('/locations/admin/hostels', data); return res.data.data;
  } catch (err) { return rejectWithValue(err.response?.data?.message); }
});

export const editHostel = createAsyncThunk('hostel/editHostel', async ({ id, data }, { rejectWithValue }) => {
  try {
    const res = await api.put(`/locations/admin/hostels/${id}`, data); return res.data.data;
  } catch (err) { return rejectWithValue(err.response?.data?.message); }
});

export const deleteHostel = createAsyncThunk('hostel/deleteHostel', async (id, { rejectWithValue }) => {
  try {
    await api.delete(`/locations/admin/hostels/${id}`); return id;
  } catch (err) { return rejectWithValue(err.response?.data?.message); }
});

export const toggleHostelStatus = createAsyncThunk('hostel/toggleStatus', async (id, { rejectWithValue }) => {
  try {
    const res = await api.patch(`/locations/admin/hostels/${id}/status`); return res.data.data;
  } catch (err) { return rejectWithValue(err.response?.data?.message); }
});
...
const hostelSlice = createSlice({
  name: 'hostel',
  initialState: {
    hostels: [], dropdownColleges: [], 
    totalHostels: 0, activeHostels: 0,
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchPublicColleges.fulfilled, (state, action) => { state.dropdownColleges = action.payload; })
