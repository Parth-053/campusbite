...
const HostelFilters = ({
  ...
  const availableColleges = adminColleges.filter(
    (c) => c.district?._id === filterDistrict
  );
  ...
  return (
    <div className="bg-white p-3 md:p-4 rounded-xl shadow-sm border border-gray-200 mb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 w-full">
        ...
        <select value={filterDistrict} onChange={(e) => { setFilterDistrict(e.target.value); setFilterCollege(""); }} className="w-full border border-gray-300 p-2 rounded-lg text-xs md:text-sm focus:ring-2 focus:ring-primary outline-none disabled:bg-gray-50 disabled:text-gray-400 bg-white" disabled={!filterState}>
          <option value="">All Districts</option>
          {districts.map((d) => (<option key={d._id} value={d._id}>{d.name}</option>))}
        </select>

        <select value={filterCollege} onChange={(e) => setFilterCollege(e.target.value)} className="w-full border border-gray-300 p-2 rounded-lg text-xs md:text-sm focus:ring-2 focus:ring-primary outline-none disabled:bg-gray-50 disabled:text-gray-400 bg-white" disabled={!filterDistrict}>
