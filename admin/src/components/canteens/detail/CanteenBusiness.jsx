  ...
  <div className="space-y-6 animate-in fade-in duration-300">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 p-6 rounded-xl text-white shadow-md relative overflow-hidden">
        ...
        <h3 className="text-3xl font-bold relative z-10">{canteen.totalOrders.toLocaleString()}</h3>
      </div>
      <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 p-6 rounded-xl text-white shadow-md relative overflow-hidden">
        ...
        <h3 className="text-3xl font-bold relative z-10">${canteen.revenue.toLocaleString()}</h3>
      </div>
    </div>

    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
      <h3 className="text-base font-bold text-gray-800 mb-4">Withdrawal History</h3>
      {canteen.transactions.length > 0 ? (
        <div className="space-y-3">
          {canteen.transactions.map(tx => (
            <div key={tx.id} className="flex justify-between items-center p-3 border border-gray-100 rounded-lg hover:bg-gray-50">
              <div>
