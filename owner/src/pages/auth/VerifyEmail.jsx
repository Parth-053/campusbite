...
const VerifyEmail = () => {
  ...
  const { verify, resendOtp, isLoading, error, tempEmail } = useAuth();
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-slate-100 text-center animate-in zoom-in-95 duration-300">
      ...
      <p className="text-slate-500 mb-6 text-sm">We've sent a 6-digit code to <br/><span className="font-semibold text-slate-800">{tempEmail || 'your email'}</span></p>

      {error && <div className="mb-4 text-red-600 text-sm bg-red-50 p-2 rounded">{error}</div>}
      {resendMsg && <div className="mb-4 text-green-600 text-sm bg-green-50 p-2 rounded">{resendMsg}</div>}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" className="btn-primary w-full py-3" disabled={isLoading || otp.length < 6}>
          {isLoading ? <span className="flex items-center justify-center gap-2"><Loader2 className="animate-spin" /> Verifying...</span> : 'Verify Account'}
        </button>
      </form>
      <button onClick={handleResend} type="button" className="mt-4 text-sm text-slate-400 hover:text-primary underline" disabled={isLoading}>
        Resend Code
      </button>
    </div>
  );
